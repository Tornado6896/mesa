                    ...
                    const struct ir3_compiler_options *options)
{
   ...
   /* ============================================================
    * 1. СНАЧАЛА загружаем базовые опции NIR
    * ============================================================ */
   compiler->nir_options = ir3_base_options;

   /* ============================================================
    * 2. Базовые значения компилятора
    * ============================================================ */
   ...
   if (compiler->gen >= 6) {
      ...
      /* ============================================================
       * A8XX: РАСШИРЕННЫЕ ОПТИМИЗАЦИИ (теперь не будут перезаписаны!)
       * ============================================================ */
      if (compiler->gen == 8) {
         /* 1. Увеличенный стек для двойной волны */
         compiler->branchstack_size = dev_info->props.has_dual_wave_dispatch ? 1024 : 512;
         compiler->max_branchstack = 128;

         /* 2. Агрессивное развертывание циклов */
         compiler->nir_options.max_unroll_iterations = 128;

         /* 3. Увеличенный safe константный лимит для A8XX */
         compiler->max_const_safe = 128;

         /* 4. Полная поддержка инструкций Dot Product */
         if (dev_info->props.has_dp2acc || dev_info->props.has_dp4acc) {
            ...
            if (dev_info->props.has_compliant_dp4acc) {
               compiler->nir_options.has_sdot_4x8 = true;
         ...
         if (!(ir3_shader_debug & IR3_DBG_NOFP16)) {
            ...
            compiler->nir_options.has_icsel_eqz16 = true;
   ...
   /* ============================================================
    * 3. Остальные NIR настройки (не перезаписываем базовые)
    * ============================================================ */
