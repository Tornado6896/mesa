...
/* Для всех чипов A8xx, включая A825, A829, A830, A840.
 * Параметры подтверждены для A825 (тесты стабильны при отключённом LRZ).
 * Размеры буферов и метаданных соответствуют аппаратным требованиям.
 */
template <>
struct PACKED fd_lrzfc_layout<A8XX> {
   ...
   union {
      struct {
         ...
         /* br_cur_buffer is repeated for slice[1..n-1], offset
          * of per-slice value is 64B*slice_num
          */
         uint8_t _padding3[1024-64];
      };
...
/* Дополнительная проверка размера структуры для A8XX.
 * Если размер изменится, компиляция упадёт с ошибкой.
 */
static_assert(sizeof(fd_lrzfc_layout<A8XX>) == 32576,
              "Unexpected size of LRZ fast-clear buffer for A8XX");
